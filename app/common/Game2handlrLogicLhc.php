...
class Game2handlrLogicLhc {
  ...
  public function regex_betV2($content) {
    ...
    $bet_str_clr = \strspc\convtCnSpace($content);
    $lineNumStr = " mm:))" . __METHOD__ . "() sa " . __FILE__ . ":" . __LINE__;
    \think\facade\Log::betnotice($lineNumStr);
    \think\facade\Log::betnotice(" bet_str_arr_clr:" . json_encode($bet_str_clr, JSON_UNESCAPED_UNICODE));
    \think\facade\Log::info("250L");
    \think\facade\Log::info(json_encode($bet_str_clr));
